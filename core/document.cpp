...
void DocumentPrivate::sendGeneratorPixmapRequest()
{
    ...
    if (tm) {
        ...
        pixmapBytes = 4l * request->width() * request->height();
    }
...
void DocumentPrivate::rotationFinished(int page, const Okular::Page *okularPage)
{
...
void Document::closeDocument()
{
    ...
    if (d->m_generator && d->m_pagesVector.size() > 0) {
        ...
        for (const Page *p : qAsConst(d->m_pagesVector)) {
    ...
    if (key == QLatin1String("NamedViewport") && option.toString().startsWith(QLatin1String("src:"), Qt::CaseInsensitive) && d->m_synctex_scanner) {
        ...
        if (synctex_display_query(d->m_synctex_scanner, QFile::encodeName(name).constData(), line, -1, 0) > 0) {
            ...
            while ((node = synctex_scanner_next_result(d->m_synctex_scanner))) {
                ...
                vp.pageNumber = synctex_node_page(node) - 1;
                ...
                if (vp.pageNumber >= 0) {
                    ...
                    vp.rePos.normalizedX = px / page(vp.pageNumber)->width();
                    vp.rePos.normalizedY = (py + 0.5) / page(vp.pageNumber)->height();
...
QPageLayout::Orientation Document::orientation() const
{
    ...
    for (uint i = 0; i < pages(); i++) {
        const Okular::Page *thisPage = page(i);
        double width = thisPage->width();
        double height = thisPage->height();
        if (thisPage->orientation() == Okular::Rotation90 || thisPage->orientation() == Okular::Rotation270) {
...
void DocumentPrivate::textGenerationDone(const Page *page)
{
    if (!m_pageController) {
        return;
    }
