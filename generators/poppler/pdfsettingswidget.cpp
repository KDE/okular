    ...
    : QWidget(parent)
{
    ...
    if (!backends.empty()) {
        ...
        connect(m_pdfsw.kcfg_EnablePgp, &QAbstractButton::toggled, this, [this](bool checked) {
            bool wasAllowed = Poppler::isPgpSignaturesAllowed();
            if (!wasAllowed && checked) {
                KMessageBox::information(
                    this, i18nc("@info Kind of a notice/warning", "These signatures only work between modern versions of Okular"), i18nc("@title:dialog", "Enable PGP Signatures"), QStringLiteral("enablePgpWarningShown"));
            }
