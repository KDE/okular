...
void WelcomeItem::urlOpened(const QUrl &url)
{
    ...
    for (int r = 1; r < numRows; r++) {
        ...
        const QUrl _url = m_recentItemsModel->data(m_recentItemsModel->index((r - 1), 0), Qt::ItemDataRole::DisplayRole).toUrl();
    ...
    cg.writePathEntry(QStringLiteral("File%1").arg(numRows), url.toDisplayString(QUrl::PreferLocalFile));
    cg.writePathEntry(QStringLiteral("Name%1").arg(numRows), shortName);
}
