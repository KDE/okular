...
struct SignatureItem
{
    ...
    SignatureItem();
    SignatureItem( SignatureItem *parent, Okular::FormFieldSignature *form, DataType type, int page );
    ~SignatureItem();

    QVector<SignatureItem *> children;
    SignatureItem *parent;
    Okular::FormFieldSignature *form;
...
SignatureItem::SignatureItem()
    : parent( nullptr ), form( nullptr ), type( Null ), page( -1 )
{
}

SignatureItem::SignatureItem( SignatureItem *_parent, Okular::FormFieldSignature *_form , DataType _type, int _page )
...
void SignatureModelPrivate::rebuildTree( const QVector<Okular::Page *> &pages )
{
    ...
    foreach ( auto page, pages )
    {
        ...
        QVector<Okular::FormFieldSignature*> signatureFormFields = SignatureGuiUtils::getSignatureFormFields( document, false, currentPage );
        if ( signatureFormFields.isEmpty() )
            continue;

        for ( int i = 0; i < signatureFormFields.count(); i++ )
        {
            Okular::FormFieldSignature *sf = signatureFormFields[i];
            Okular::SignatureInfo *info = sf->validate();
            ...
            childItem2->displayString = i18n("Signing Time: %1", info->signingTime().toString( QStringLiteral("MMM dd yyyy hh:mm:ss") ) );
...
QVariant SignatureModel::data( const QModelIndex &index, int role ) const
{
    ...
    switch ( role )
    {
        ...
        case Qt::DecorationRole:
            ...
            {
                Okular::SignatureInfo *info = item->form->validate();
            ...
            return QIcon();
        case FormIdRole:
            return item->form ? item->form->id() : -1;
        case PageRole:
            return item->page;
    }
